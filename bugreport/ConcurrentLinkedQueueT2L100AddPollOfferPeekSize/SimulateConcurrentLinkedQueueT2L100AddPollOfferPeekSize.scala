...
class SimulateConcurrentLinkedQueueT2L100AddPollOfferPeekSize(logName: String, verbose: Boolean) extends Simulation(logName, verbose) { 
  ...
  def sequentialExecute (tr: List[(Int, Int)]) (init: T) = { 
    ...
    for ((tid, midx) <- tr; if consistent ) { 
      ...
      mEvent.methodID match { 
        ...
        case 2 => { 
          val arg__0= mEvent.arguments(0).toInt 
          val ret = obj.offer(arg__0)
          if (ret != mEvent.retValue.toBoolean) { consistent = false } 
        } 
        case 3 => { 
          val ret = Option(obj.peek()) 
          ret match { 
            case Some(x) => if (x != mEvent.retValue.toInt) { consistent = false }  
            case None => if (mEvent.retValue.toInt >= 0) { consistent = false }  
          } 
...
object Simulate { 
  ...
  val simu : Simulation = new SimulateConcurrentLinkedQueueT2L100AddPollOfferPeekSize(logName, verbose) 
  val finalState = simu.execute () 
  if (! finalState.isEmpty) println("OK") 
  } 
} 
