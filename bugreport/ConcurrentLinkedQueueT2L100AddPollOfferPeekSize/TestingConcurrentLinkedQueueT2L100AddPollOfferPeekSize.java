// 0: add(int):boolean; 1: poll():obj[int]; 2: offer(int):boolean; 3: peek():obj[int]; 4: size():int; 
  ...
  public void run() { 
    ...
    for (int i = 0; i < len; i++) { 
      ...
      case 1: 
        ...
        break ; 
      case 2: 
        offer__1 = elemBase + r.nextInt(1000) ; 
        args.add(new ArgInt(offer__1)) ; 
        offer__ret = data.offer(offer__1) ; 
        trace[i] = new TraceRecord(args, offer__ret, mid) ; 
        break ; 
      case 3: 
        peek__ret = data.peek() ; 
        if (peek__ret == null) trace[i] = new TraceRecord(args, -1, mid) ; 
        else trace[i] = new TraceRecord(args, Integer.parseInt(peek__ret.toString()), mid) ; 
        break ; 
      case 4: 
        size__ret = data.size() ; 
...
class ConcurrentLinkedQueueT2L100AddPollOfferPeekSize { 
  public static void main(String argv[]) { 
    int tdNum = 2, mdNum = 5, trLen = 100; 
    ...
    try { 
      ...
      out.write("2 100 ConcurrentLinkedQueue add poll offer peek size\n") ; 
      for (int i=0; i < tdNum; i++) { 
        out.write("Thread " + i + "\n") ; 
